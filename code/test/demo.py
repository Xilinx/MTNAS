...
parser.add_argument('--Detarch', type=str, default='DetDARTS', help='which Det architecture to use')
parser.add_argument('--Segarch', type=str, default='SegDARTS', help='which Seg architecture to use')
...
def get_voc_results_file_template(cls):
    ...
    filedir = os.path.join(args.save_folder, 'demo')
...
def test_net(save_folder, net, cuda, ids, detect, transform, thresh=0.01, has_ori=False):
    ...
    save_seg_root = os.path.join(save_folder, 'demo')
    ...
    for i in range(num_images):
        ...
        if args.cuda:
            x = x.cuda()
        _t['im_detect'].tic()
        loc_ori, conf_data, seg_data, priors = net(x)
...
if __name__ == '__main__':
    ...
    Detgenotype = eval("genotypes.%s" % args.Detarch)
    Seggenotype = eval("genotypes.%s" % args.Segarch)
    net = model_darts14.build_model(args.init_channels, args.layers, genotype,Detgenotype,Seggenotype,det_classes,seg_classes)
